...
<template>
  <section class="relative rounded-[32px] surface-panel p-6 sm:p-8">
    <div aria-hidden="true" class="pointer-events-none absolute inset-0 rounded-[32px] border border-panel-soft">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
      <div class="space-y-2">
        ...
        <p class="max-w-2xl text-xs leading-relaxed text-muted sm:text-sm">
    ...
    <div v-if="!isInteractiveMode" class="mt-6">
      <div
        class="canvas-shell relative overflow-hidden rounded-[28px] border border-panel bg-overlay-strong shadow-[0_40px_90px_-45px_rgba(15,23,42,0.9)]"
    ...
    <template v-else>
      <Teleport to="body">
        <div
          class="fixed inset-0 z-50 flex items-start justify-center bg-overlay px-4 py-8 sm:py-10 backdrop-blur-xl overflow-y-auto lg:items-center lg:overflow-visible"
          @click.self="closeInteractiveMode"
        >
          <button
            class="absolute right-6 top-6 inline-flex items-center gap-2 rounded-full border border-panel-strong bg-panel px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-primary-soft transition hover:border-panel-strong hover:text-white"
            type="button"
          ...
          <div class="grid w-full max-w-[min(100vw-2rem,1600px)] items-start gap-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,0.8fr)] xl:gap-8">
            ...
            <div class="flex flex-col gap-4 rounded-[24px] border border-panel bg-overlay p-4 text-[11px] text-primary">
              <div class="space-y-1">
              ...
              <section class="rounded-2xl border border-panel bg-panel p-3">
                <div class="flex items-center justify-between text-[10px] uppercase tracking-[0.28em] text-muted">
                  <span>{{ t('home.hero.interactive.portrait.title') }}</span>
                  <span>{{ t('home.hero.interactive.portrait.type') }}</span>
                </div>
                <div class="mt-3 grid gap-2 sm:grid-cols-2">
                  <label class="flex flex-col gap-1 text-[10px] uppercase tracking-[0.26em] text-dim">
                    {{ t('home.hero.interactive.portrait.fields.left') }}
                    <input
                      v-model.number="accentPortraitImage.left"
                      class="rounded-lg border border-panel bg-panel-soft px-2 py-1.5 text-xs text-primary"
                      max="580"
                      min="0"
                      step="1"
                      type="number"
              ...
              <section class="rounded-2xl border border-panel bg-panel p-3">
                ...
                <div class="mt-3 space-y-2">
                  ...
                  <template v-if="activeTextLayer">
                    <label class="flex flex-col gap-1 text-[10px] uppercase tracking-[0.26em] text-dim">
                      ...
                      <textarea
                        ...
                        class="min-h-[52px] rounded-lg border border-panel bg-panel-soft px-2 py-1.5 text-xs text-primary"
                      />
                    </label>
                    <div class="grid gap-2 sm:grid-cols-2">
                      ...
                      <label class="flex flex-col gap-1 text-[10px] uppercase tracking-[0.26em] text-dim">
                        ...
                        <input
                          ...
                          class="h-8 rounded-lg border border-panel bg-panel-soft px-2 py-1 text-xs text-primary"
                          type="color"
                        >
                      </label>
                    </div>
                  ...
                  <p v-else class="text-[10px] text-dim">
