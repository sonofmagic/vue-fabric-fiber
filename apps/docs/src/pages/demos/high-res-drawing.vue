...
import HighResDrawingDemo from '@/components/demos/HighResDrawingDemo.vue'
import { buildCanonicalUrl, usePageSeo } from '@/seo'
import { demoCards } from './cards'

const slug: DemoCardSlug = 'hiResDrawing'
const definition = demoCards.find(entry => entry.slug === slug)

if (!definition) {
  throw new Error('Missing demo card metadata for /demos/high-res-drawing')
}
